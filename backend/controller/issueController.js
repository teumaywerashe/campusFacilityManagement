...
export const createIssue = async (req, res) => {
  try {
    ...
    const newNotification = await Notification.create({
      receiverId: userId,
      content: `We have received your report: "${content.substring(
        0,
        20,
      )}...". Thank you.`,
      reportId: issue._id,
...
export const getAllIssues = async (req, res) => {
  try {
    const issues = await Issue.find()
      .sort({ createdAt: -1 })
      .populate("userId", "name email") // populate the issue author
      .populate({
        path: "comments", // populate virtual comments
        populate: { path: "userId", select: "name email" }, // populate comment author
      });
...
export const getUserIssues = async (req, res) => {
  try {
    ...
    const issues = await Issue.find({ userId: id })
      .sort({ createdAt: -1 })
      .populate("userId", "name email") // issue author
      .populate({
        path: "comments", // populate virtual comments
        populate: { path: "userId", select: "name email" }, // populate comment author
      })
